...
Indeed, the mixing matrix shown in table \ref{tab:mixmat} indicates a bias towards forming ties between proteins when they hold different classes (i.e. non-lethal with lethal). 
...
By definition, nodefactor adds multiple statistics to the model, each one equal to the number of times a node with the specified attribute is at one end of an edge. To interpret the results in summary statistics of the model, it is necessary to know the reference group for any categorical variables. In the statnet package, the reference group in an ERGM defaults to the first group in the list shown in the summary of the network. In our case, the class "lethal" is the reference group for the attribute "lethality". Since the "non-lethal" group contains the most nodes and, by extension, the highest degree, we have set it as the reference group.
...
```{r diag, echo = FALSE, fig.align='center', fig.width=7, fig.height=6, fig.cap="Diagnostic plots of the goodness of fit", warnings = FALSE}
par(mfrow = c(2,2))
gof.model = gof(ergm.fit.1)
plot(gof.model, cex.main = 0.6)
```

**Analysis of the diagnostics plot :**
