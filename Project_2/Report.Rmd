...
The ERGM is a statistical model that allows us to analyse a network. We model the dependent variable $Y$ as a member of the exponential family. Mathematically, we have the following^[Notation from the slides of Eftychia Solea (UE-MSD01), Topic 4]:
...
knitr::kable(
 df, col.names = c("Diameter", "Average shortest path", "Transitivity (Clustering Coefficient)")
)
```
...
* ERGM where both interaction terms as well as main effects are considered.

As means of model comparison and model goodness of fit study, we will look at the Bayesian Information Criterion (BIC) and the diagnostic plots produced by the function *gof* of the *ergm* package in *R*.
...
By definition, nodefactor adds multiple statistics to the model, each one equal to the number of times a node with the specified attribute is at one end of an edge. To interpret the results in summary statistics of the model, it is necessary to know the reference group for any categorical variables. In the statnet package, the reference group in an ERGM defaults to the first group in the list shown in the summary of the network. 

In our case, the class "lethal" is the reference group for the attribute "lethality" by default. Since the "non-lethal" group contains the most nodes and, by extension, the highest degree, we have set it as the reference group.
...
Summary statistics of the last model along with figure \ref{fig:diag3} show a good fit except the proportion of statistics across the lethality attribute and an over estimation of the proportion of nodes. In contrast, it yields the lowest BIC score out of all the models tested. 
