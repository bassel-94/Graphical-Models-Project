...
#-- preprocessing steps to avoid having many clusters
load("protein_net.Rda")

#-- summary stats
summary(protein_net, print.adj = FALSE)
```

Descriptive statistics shows that we have three attributes in the network. The number, the group and lethality. Both number and group attributes are not of interest to us for this project and we will, therefore, focus on the lethality. It is worth mentioning that the density of the model is around 0.002.
...
legend("bottomright", legend=c("Unknown", "Non-Lethal", "Lethal", "Other"), col = my.pal, pch = 19, pt.cex=2)
```
...
smallnet = as.network(prot_small, directed=FALSE)
...
plot( smallnet, vertex.col = l, edge.col = "grey40", main = "Interaction among the proteins with different sizes", vertex.cex =0.7)
legend("bottomright", legend=c("Unknown", "Non-Lethal", "Lethal", "Other"), col = my.pal, pch = 19, pt.cex=2)
```
...
plot(protein_net, vertex.col = l, edge.col = "grey40", main = "Interaction among the proteins", vertex.cex = n/20)
legend("bottomright", legend=c("Unknown", "Non-Lethal", "Lethal", "Other"), col = my.pal, pch = 19, pt.cex=2)
```
